...
print("=" * 60)
print("MarkShot - Marker Frame Export Tool v1.3.0")
print("=" * 60)
...
selected_color = "All"
use_burnin = False  # Default OFF for Free version compatibility
...
try:
    ...
    print(f"Note: Using default settings (JPEG, All markers, Burn-in OFF)")
    print(f"  (UIManager error: {e})")
    selected_format = "JPEG"
    selected_color = "All"
    use_burnin = False  # OFF for Free version compatibility
...
for i, (frame_id, marker_info) in enumerate(sorted(filtered_markers.items()), 1):
    try:
        # Calculate timecode (frame_id is offset from timeline start)
        timecode = frame_to_timecode(int(frame_id), start_tc, fps)
        tc_label = timecode.replace(":", "_")

        # Move playhead
        timeline.SetCurrentTimecode(timecode)
        time.sleep(0.5)  # Wait for playhead to move and display to update
